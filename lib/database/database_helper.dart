...
class DatabaseHelper {
  ...
  void _createDb(Database db, int version) async {
    await db.execute(
        'CREATE TABLE $taskTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colTitle TEXT, $colisDone TEXT)');
  }
  ...
  Future<List<TaskTile>> getTaskList() async {
    final List<Map<String, dynamic>> taskMapList = await getTaskMapList();
    final List<TaskTile> taskList = [];
    taskMapList.forEach((taskMap) {
      taskList.add(TaskTile.fromMap(taskMap));
    });
    //taskList.sort((taskA, taskB) => taskA.date.compareTo(taskB.date));
    return taskList;
  }

  Future<int> insertTask(TaskTile task) async {
    Database db = await this.db;
    final int result = await db.insert(taskTable, task.toMap());
    return result;
  }

  Future<int> updateTask(TaskTile task) async {
    Database db = await this.db;
